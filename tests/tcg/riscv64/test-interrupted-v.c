...
{
	unsigned long page = (unsigned long)info->si_addr & ~(unsigned long)(page_size - 1);
...
static __attribute__((noinline)) uint8_t unit_load(uint8_t *mem, size_t nr, bool ff)
{
	size_t vl;
...
static __attribute__((noinline)) uint8_t seg2_load(uint8_t *mem, size_t nr, bool ff)
{
	size_t vl;
	vuint8m1x2_t segvec;
	vuint8m1_t vec, redvec, sum;
...
static __attribute__((noinline)) uint8_t strided_load(uint8_t *mem, size_t nr, size_t stride)
{
	size_t vl;
	vuint8m1_t vec, redvec, sum;
...
static __attribute__((noinline)) uint8_t indexed_load(uint8_t *mem, size_t nr, uint32_t *indices)
{
	size_t vl;
	vuint32m4_t idx;
	vuint8m1_t vec, redvec, sum;
...
static int run_interrupted_v_tests(void)
{
	...
	mem = mmap(NULL, NR_ELEMS * page_size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
	...
	assert(unit_load(&mem[page_size - (NR_ELEMS / 2)], NR_ELEMS, false) == 8 * NR_ELEMS / 2);
	assert(nr_segv == 2);
	...
	mprotect(mem, page_size * 2, PROT_NONE);
	assert(seg2_load(&mem[page_size - NR_ELEMS], NR_ELEMS, false) == 8 * NR_ELEMS);
	assert(nr_segv == 2);
	...
	fault_end = fault_start + (NR_ELEMS / 2);
	mprotect(mem, page_size * 2, PROT_NONE);
	assert(unit_load(&mem[page_size - (NR_ELEMS / 2)], NR_ELEMS, true) == 3 * NR_ELEMS / 2);
	...
	fault_end = fault_start + NR_ELEMS;
	mprotect(mem, page_size * 2, PROT_NONE);
	assert(seg2_load(&mem[page_size - NR_ELEMS], NR_ELEMS * 2, true) == 3 * NR_ELEMS);
	assert(nr_segv == 1); /* fault-first does not fault the second page */
